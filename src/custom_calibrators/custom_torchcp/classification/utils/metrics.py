...
@METRICS_REGISTRY_CLASSIFICATION.register()
def multiclass_brier_score_loss(prediction_probs, labels):
    # Convert list of tensors to NumPy arrays, handling zero-dimensional tensors
    prediction_probs_np = np.concatenate(
        [tensor.cpu().numpy().reshape(1, -1) if tensor.ndim == 0 else tensor.cpu().numpy() for tensor in prediction_probs],
        axis=0
    )
    labels_np = np.concatenate(
        [tensor.cpu().numpy().reshape(1) if tensor.ndim == 0 else tensor.cpu().numpy() for tensor in labels],
        axis=0
    )
    
    # Calculate Brier score for each class and average them
    brier_conformal = np.mean([
        sklearn_brier_score_loss(labels_np == i, prediction_probs_np[:, i])
        for i in range(prediction_probs_np.shape[1])
    ])

    return brier_conformal



@METRICS_REGISTRY_CLASSIFICATION.register()
def log_loss(prediction_probs, labels):
    # Convert list of tensors to NumPy arrays, handling zero-dimensional tensors
    prediction_probs_np = np.concatenate(
        [tensor.cpu().numpy().reshape(1, -1) if tensor.ndim == 0 else tensor.cpu().numpy() for tensor in prediction_probs],
        axis=0
    ...
    # Calculate log loss
    return sklearn_log_loss(labels_np, prediction_probs_np)




@METRICS_REGISTRY_CLASSIFICATION.register()
def unilabel_set_pct(prediction_sets, labels):
    labels = labels.cpu()
    nb_sets = 0
    for index, ele in enumerate(prediction_sets):
        if len(ele) == 1:
            nb_sets += 1
    return nb_sets / len(prediction_sets)