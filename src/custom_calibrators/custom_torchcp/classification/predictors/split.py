...
class SplitPredictor(BasePredictor):
    ...
    def predict_probabilities(self, x_batch):
        ...
        x_batch = self._logits_transformation(x_batch).detach()
        probabilities = F.softmax(x_batch, dim=1)
    ...
    def evaluate(self, val_dataloader):
        ...
        with torch.no_grad():
            for examples in val_dataloader:
                ...
                probs_sets_batch = self.predict_probabilities(tmp_x)
                probs_sets.append(probs_sets_batch)
        ...
        res_dict = {"Coverage_rate": self._metric('coverage_rate')(prediction_sets, val_labels),
                    ...
                    "Unilable_share":self._metric('unilabel_set_pct')(prediction_sets, val_labels),
                    "Multiclass_brier_score":self._metric('multiclass_brier_score_loss')(probs_sets, val_labels),
                    "Log_loss":self._metric('log_loss')(probs_sets, val_labels)}
        return res_dict
